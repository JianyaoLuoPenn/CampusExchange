...
补齐从源码构建的 Docker 启动、私密环境变量生成、健康检查、前端 API 代理、VS Code 正确入口和重启验证；修复了全新容器中 Vite 写权限、localhost/127.0.0.1 登录跨域、旧 JWT 阻塞公共页面等实际问题。还补测并修复支付模式切换、长图片 URL、金额输入与确定性退款请求失败。这些是对前次交付缺口的真实修复，不把原来的 CI 通过等同于本地可复现启动。
