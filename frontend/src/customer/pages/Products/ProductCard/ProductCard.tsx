...
export function ItemArt({ item }: { item: Listing }) {
  const Icon =
  ...
  return (
    <div
      className={"item-art art-" + item.category.split(" ")[0].toLowerCase()}
    >
      {item.images[0] ? (
        <img
          src={item.images[0]}
  ...
  return (
    <Link to={"/listings/" + item.id} className="product-card">
      ...
      <div className="card-body">
        ...
        <h3>{item.title}</h3>
        <strong className="price">{money(item.priceCents)}</strong>
        <p>
          {item.apartment} · {item.campus}
        </p>
        <div className="card-foot">
          <span>{item.sellerName}</span>
          <span>
            {item.depositCents
              ? money(item.depositCents) + " deposit"
              : "No deposit"}
          </span>
        </div>
      </div>
