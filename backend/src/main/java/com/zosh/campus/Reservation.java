...
public class Reservation {
    ...
    @ManyToOne(optional=false, fetch=FetchType.LAZY) private User buyer;
    @OneToOne(optional=false) private Order order;
    @Enumerated(EnumType.STRING) private Status status;
    @Enumerated(EnumType.STRING) private PaymentState paymentState;
    ...
    @Column(unique=true) private String checkoutId;
    ...
    public enum Status { PENDING_PAYMENT, RESERVED, COMPLETED, CANCELLED, EXPIRED }
    public enum PaymentState { NOT_REQUIRED, UNPAID, FAILED, PAID, REFUND_PENDING, REFUND_FAILED, REFUNDED }
}
